...
export function ProcessingStages() {
  ...
  useEffect(() => {
    const storedRecipients = JSON.parse(localStorage.getItem('recipients') || '[]') as Recipient[];
          ...
          <React.Fragment key={index}>
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center mb-2 ${
                currentStage > index + 1 ? 'bg-green-500' : currentStage === index + 1 ? 'bg-blue-500' : 'bg-gray-300'
              }`}>
                {index + 1}
              </div>
          ...
          <tbody>
            {recipients.map((recipient, index) => (
              <tr key={index}>
                ...
                <td className="border border-gray-300 p-2">{recipient.email}</td>
                <td className="border border-gray-300 p-2">
                  {status === "Completed" ? "Completed" : ` ${stages[currentStage - 1]}`}
                </td>
              </tr>
            ))}
          </tbody>
