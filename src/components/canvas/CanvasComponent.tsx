...
const CanvasComponent = (props: ICanvasComponent) => {
  ...
  const onBlur = (_event: React.FocusEvent<HTMLDivElement>) => {
    ...
    if (
      _event.currentTarget.contains(_event?.relatedTarget as Element) ||
      toolbarElement?.contains(_event?.relatedTarget as Element)
    ) {
      return;
    }
    setIsReadOnly(true);
  ...
  const onKeyDown = (_event: React.KeyboardEvent) => {
    if (!isReadOnly) _event.stopPropagation();
  };
